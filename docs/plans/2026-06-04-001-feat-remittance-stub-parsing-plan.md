...
- R2. Stubs reconcile against Cinderhaven SSOT ($5.4M all-in trade cost)
...
- **Recoverable dollar figure:** The specific recovery rate and dollar amount derived from the 464 chargebacks / $5.4M base during U5.
...
- **Recoverable figure:** Derive from the reconciled data — sum of invalid/disputable deductions that are still within the dispute window, anchored against the $5.4M base
...
- Covers AE5. Ledger totals tie exactly to Cinderhaven $5.4M canonical figure
...
- **Unchanged invariants:** Cinderhaven SSOT figures ($5.4M all-in trade cost, 464 chargebacks) are read-only inputs — this project never writes to the SSOT
